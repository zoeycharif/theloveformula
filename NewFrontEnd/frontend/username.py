...
result = session.query(Profiles).filter(Profiles.username=='').first()
...
## What this module needs to do is check to see if the username already exists on the sql server. If it doesn't, create a new record that 
# includes just the new username, and nulls for the rest of the records.
# 
# If the username exists, then have the user try again.    